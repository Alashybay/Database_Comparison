...
def seedBooks():
    ...
    CREATE (:Book {book_id: toInteger(line.book_id), title: line.title, author: line.author, publication_date: line.publication_date})
    """)
...
def seedBorrowers():
    ...
    CREATE (:Borrower {borrower_id: toInteger(line.borrower_id), name: line.name, email: line.email})
    """)
...
def seedBorrowingHistory():
    ...
    MATCH (book:Book {book_id: toInteger(line.book_id)})
    MATCH (borrower:Borrower {borrower_id: toInteger(line.borrower_id)})
    CREATE (borrower)-[:BORROWED {borrowing_id: toInteger(line.borrowing_id), borrow_date: line.borrow_date, return_date: line.return_date}]->(book)
    """)
